...
function FeatureCard({
  ...
  return (
    <Card className="group hover:border-primary/40 transition-all duration-300 bg-transparent border-border">
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-20 pb-24 text-center">
        {/* Bahrain badge */}
        <div className="inline-flex items-center gap-2 bg-card border border-border rounded-full px-4 py-2 text-sm text-muted-foreground mb-8">
          <span>🇧🇭</span>
        ...
        <h1 className="text-4xl md:text-6xl font-bold text-foreground leading-tight
          ...
          <span className="text-primary">نظام QR للطلبات</span>
          {' '}في مطاعم البحرين
        </h1>
        ...
        <div className="mt-16 relative">
          ...
          <div className="relative bg-card border border-border rounded-3xl p-6 max-w-3xl mx-auto">
            <div className="grid grid-cols-3 gap-4 text-left">
              {/* Mock order card */}
              {['D-001', 'D-002', 'D-003'].map((num, i) => (
                <div key={num} className={`rounded-xl p-3 border-2 ${
                  i === 0 ? 'border-yellow-500/40 bg-yellow-50/50' :
                  i === 1 ? 'border-emerald-500/40 bg-emerald-50/50' :
                  'border-border bg-card'
                }`}>
                  <div className="text-xs text-muted-foreground mb-1">
                    {i === 0 ? '🍳 يتم التحضير' : i === 1 ? '✅ جاهز' : '⏳ في الانتظار'}
                  </div>
                  <div className="font-bold text-foreground text-lg">{num}</div>
                  <div className="text-xs text-muted-foreground mt-1">طاولة {i + 1}</div>
                </div>
              ))}
            </div>
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between
                  ...
                  <div>
                    ...
                    <div className="text-xs text-primary">{item.price} د.ب.</div>
                  </div>
