      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="stat-card">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="stat-label text-xs">{isAr ? 'هذا الشهر' : 'This Month'}</span>
          </div>
          <div className="stat-value text-brand-400">{formatBHD(monthlyTotal, 'en')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label text-xs mb-1">{isAr ? 'الإجمالي الكلي' : 'All Time'}</div>
          <div className="stat-value text-purple-400">{formatBHD(allTimeTotal, 'en')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label text-xs mb-1">{isAr ? 'عدد الدفعات' : 'Total Payments'}</div>
        ...
        <div className="overflow-x-auto table-scroll">
          <table className="w-full text-sm min-w-[640px]">
            <thead>
              <tr className="border-b border-[#2a2825]">
                {['Date', 'Restaurant', 'Amount', 'Method', 'Reference', 'Period', 'Notes'].map(h => (
          ...
          {rows.length === 0 && (
            <div className="text-center py-16 text-[#57534e]">
              <p>{isAr ? 'لا توجد مدفوعات بعد' : 'No payments recorded yet'}</p>
            </div>
          )}
        </div>
      </div>
