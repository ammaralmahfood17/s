...
export default async function AdminSubscriptionsPage({
  ...
  const tabs = [
    ...
    { key: 'trialing', label: isAr ? 'تجريبي' : 'Trial' },
    { key: 'past_due', label: isAr ? 'متأخر' : 'Past Due' },
    { key: 'expiring', label: isAr ? 'ينتهي قريباً' : 'Expiring Soon' },
  ];
    ...
    <div className="p-4 sm:p-6 max-w-6xl space-y-4">
      <div>
        ...
        <p className="text-sm text-[#57534e]">
          {rows.length} {isAr ? 'اشتراك' : 'subscriptions'}{filter ? ` · ${filter}` : ''}
        </p>
      </div>
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        {tabs.map(tab => (
          <Link key={tab.key}
            href={`/${locale}/admin/subscriptions${tab.key ? `?filter=${tab.key}` : ''}`}
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead>
              <tr className="border-b border-[#2a2825]">
                {['Restaurant', 'Plan', 'Status', 'Period End', 'Days Left', 'Amount', 'Actions'].map(h => (
                ...
                return (
                  <tr key={s.id} className={`border-b border-[#1a1916] hover:bg-[#1a1916] transition-colors ${isExpired ? 'opacity-60' : ''}`}>
                    <td className="px-4 py-3">
                      <div className="font-medium text-[#fafaf9]">{r?.name_en ?? '—'}</div>
                      <div className="text-xs text-[#57534e] font-mono">{r?.slug}</div>
                    </td>
                    <td className="px-4 py-3 text-xs text-[#a8a29e]">
                      {s.plans?.name_en ?? '—'}
                      <div className="text-[#57534e]">{s.plans?.price_bhd === 0 ? 'Free' : `BHD ${s.plans?.price_bhd}/mo`}</div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-xs text-[#a8a29e]">
                      {s.amount_bhd ? formatBHD(s.amount_bhd, 'en') : '—'}
                    </td>
                    <td className="px-4 py-3">
